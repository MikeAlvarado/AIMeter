...
final class UsageHistoryStoreTests: XCTestCase {
    ...
    func testAppendsSamplesInOrder() {
        ...
        store.record(snapshot(10, resetsAt: now.addingTimeInterval(3600)), for: "claude", at: now)
        store.record(snapshot(20, resetsAt: now.addingTimeInterval(3600)), for: "claude", at: now.addingTimeInterval(600))
    ...
    func testResetDropDiscardsPriorSamples() {
        let store = UsageHistoryStore(userDefaults: defaults)
        store.record(snapshot(80, resetsAt: now.addingTimeInterval(3600)), for: "claude", at: now)
        // Big drop → new window → prior samples discarded, only the new one.
        store.record(snapshot(5, resetsAt: now.addingTimeInterval(5 * 3600)), for: "claude", at: now.addingTimeInterval(600))
    ...
    func testSmallDecreaseWithinToleranceKeepsHistory() {
        let store = UsageHistoryStore(userDefaults: defaults)
        store.record(snapshot(50, resetsAt: now.addingTimeInterval(3600)), for: "claude", at: now)
        // −4 is under the 10-point reset threshold: kept (noise, not a reset).
        store.record(snapshot(46, resetsAt: now.addingTimeInterval(3600)), for: "claude", at: now.addingTimeInterval(600))
    ...
    func testBoundedToMaxSamples() {
        ...
        for i in 0..<(cap + 10) {
            ...
            store.record(snapshot(used, resetsAt: now.addingTimeInterval(3600)),
                         for: "claude", at: now.addingTimeInterval(Double(i) * 60))
        }
        XCTAssertEqual(store.samples(for: "claude", kind: .session).count, cap)
    }

    func testScopedPerAccountAndCleared() {
        let store = UsageHistoryStore(userDefaults: defaults)
        store.record(snapshot(10, resetsAt: now.addingTimeInterval(3600)), for: "claude", at: now)
    ...
    func testTwoAccountsOfTheSameProviderDoNotCollide() {
        // Both snapshots share providerID "claude" — only the explicit
        // accountID keeps their histories from clobbering each other.
        let store = UsageHistoryStore(userDefaults: defaults)
        store.record(snapshot(10, resetsAt: now.addingTimeInterval(3600)), for: "claude", at: now)
        store.record(snapshot(90, resetsAt: now.addingTimeInterval(3600)), for: "work-uuid", at: now)

        XCTAssertEqual(store.samples(for: "claude", kind: .session).map(\.usedPct), [10])
        XCTAssertEqual(store.samples(for: "work-uuid", kind: .session).map(\.usedPct), [90])
    }
    ...
    func testObservingSinceSetOnceAndSurvivesResets() {
        ...
        store.record(snapshot(80, resetsAt: now.addingTimeInterval(3600)), for: "claude", at: now.addingTimeInterval(600))
        store.record(snapshot(5, resetsAt: now.addingTimeInterval(5 * 3600)), for: "claude", at: now.addingTimeInterval(1200))
        XCTAssertEqual(store.observingSince(for: "claude"), now)
